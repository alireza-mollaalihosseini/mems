...
def build_state_matrix(train_file_list_path, val_file_list_path, test_file_list_path, a, u_dc, mu):
    ...
    # Simulation params
    alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa = 749.37, 14000.0, 87964.6, 43.2, 0.001, 1006.6, 4.2588e7, 25.0, 1e6
    u_max = 1.0
    h = 1e-6 * omega_0
    T = 50.0 * omega_0
...
if __name__ == '__main__':
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/design1/results"
    ...
    # save state matrices
    np.savez_compressed(f"/scratch/almo2783/scratch/design1/state-matrices/train_state_a_{a:.2f}_u_dc_{u_dc:.2f}.npz", state_matrix[:len(labels_train)])
    np.savez_compressed(f"/scratch/almo2783/scratch/design1/state-matrices/val_state_a_{a:.2f}_u_dc_{u_dc:.2f}.npz", state_matrix[len(labels_train):len(labels_train)+len(labels_val)])
    np.savez_compressed(f"/scratch/almo2783/scratch/design1/state-matrices/test_state_a_{a:.2f}_u_dc_{u_dc:.2f}.npz", state_matrix[len(labels_train)+len(labels_val):])
