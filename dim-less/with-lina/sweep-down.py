...
def derivatives(y, t, phi_dc, a, c1, c2, c3, c4):
    ...
    dot_exsi = etta
    dot_etta = -c2 * etta - exsi + psy
    temp = a * phi_ac + phi_dc
    min_term = min(temp**2, 1.0)
    dot_psy = -c1 * psy + c1 * min_term
    dot_phi_ac = -c3 * phi_ac + c4 * etta
...
@njit
def simulate(y0, t_max, h, phi_dc, a, c1, c2, c3, c4, omega_0):
    n_steps = int(t_max * omega_0 / h)
...
for i, a in enumerate(a_values[::-1]):
    t, y = simulate(y0, t_max, h, phi_dc, a, c1, c2, c3, c4, omega_0)
    # results[i, :] = y[:, 0][-10000:]
...
np.save(f'max_peaks-down-kick-{kick}.npy', max_peaks)
np.save(f'min_peaks-down-kick-{kick}.npy', min_peaks)
