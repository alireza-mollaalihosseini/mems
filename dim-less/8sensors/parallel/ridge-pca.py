...
def process_lambda(lam, X_train, labels_train, X_test, labels_test, X_val, labels_val, results_dir):
    ...
    # Save results
    np.savetxt(f"{results_dir}/results_test-lam-{lam:.0e}-pca.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix_test-lam-{lam:.0e}-pca.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"{results_dir}/results_val-lam-{lam:.0e}-pca.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix_val-lam-{lam:.0e}-pca.txt",
               cm_val, fmt="%.5f")
...
if __name__ == "__main__":
    ...
    results_dir = "/scratch/almo2783/scratch/dim-less/8sensors/parallel/results"
    ...
    processed = Parallel(n_jobs=-1, verbose=10)(
        delayed(process_lambda)(
            lam, X_train_pca, labels_train, X_test_pca, labels_test,
            X_val_pca, labels_val, results_dir
        )
