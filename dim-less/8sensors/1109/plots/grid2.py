...
crossings = np.array([1.18688327, 0.59344289, 0.39563597, 0.29672254, 0.2373784 ,
                      0.19783076, 0.1695649 , 0.14838566, 0.1318974 , 0.11868982])
lam = 1e4
mu = 1.0
typs = ['test', 'val']
# Parameters
...
# Simulation params
alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa = 12.5, 1109.0, 6968.052505662161, 57.5, 0.0050999999999999995, 196.07843137254903, 2669280.0, 12.92, 400980.0
...
def process_file(a, u_dc, typ, mu, lam):
    file_name = f'/scratch/almo2783/scratch/dim-less/8sensors/1109/results/results_{typ}-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt'
...
for typ in typs:
    ...
    flat_values = data_matrix.flatten()
    flat_indices = np.argsort(flat_values)[::-1]  # Sort descending

    # Take top 10 indices (ignoring NaNs)
    top_indices = [idx for idx in flat_indices if not np.isnan(flat_values[idx])][:10]

    # Convert flat indices back to 2D
    top_coords = [np.unravel_index(idx, data_matrix.shape) for idx in top_indices]

    # The highest value
    max_idx = top_coords[0]
    max_a = a_values[max_idx[0]]
    max_u_dc = u_dc_values[max_idx[1]]
    max_value = data_matrix[max_idx]
    ...
    # Plot the next 9 values
    for (r, c) in top_coords[1:]:
        plt.plot(
    ...
    plt.tight_layout()
    plt.savefig(f'Grid-Search-{typ}-1.png', bbox_inches='tight', dpi=300)
    plt.close()
