...
def process_lambda_value(X_train, X_val, labels_train, labels_val, lambda_value, a_value, u_dc_value, results_dir, snr_db):
    ...
    # Save results
    np.savetxt(f"{results_dir}/results-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}-snr-{snr_db}.txt",
               results.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}-snr-{snr_db}.txt",
               conf_matrix, fmt="%.5f")
...
if __name__ == "__main__":
    ...
    mu = 1.0
    snr_db_values = np.array([20.0, 10.0, 5.0, 3.0, 1.0, 0.5])

    for snr_db in snr_db_values:

        # Load training, validation
        X_train = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/state-matrix/state_matrix_train-a-{a_value}-u_dc-{u_dc_value}-mu-{mu}-snr-{snr_db}.npz")['arr_0']
        X_val   = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/state-matrix/state_matrix_val-a-{a_value}-u_dc-{u_dc_value}-mu-{mu}-snr-{snr_db}.npz")['arr_0']
        ...
        # Results dir
        results_dir = f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/lambda"
        ...
        Parallel(n_jobs=-1, backend="threading", verbose=10)(  # adjust n_jobs depending on your cluster
            delayed(process_lambda_value)(
                ...
                lambda_value, a_value, u_dc_value, results_dir, snr_db
            )
