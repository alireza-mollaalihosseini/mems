...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len, snr_db=None):
...
def build_state_matrix(file_list_path, output_path, a, u_dc, mu, snr_db=None):
    ...
    results = Parallel(n_jobs=-1, backend="threading", verbose=10)(
        delayed(process_file)(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len, snr_db=None)
...
if __name__ == '__main__':
    ...
    build_state_matrix(
        train_files,
        f'/scratch/almo2783/scratch/dim-less/8sensors/noise/state-matrix/state_matrix_train-a-{a}-u_dc-{u_dc}-mu-{mu}-snr-{snr_db}.npz',
        a, u_dc, mu, snr_db
    )

    build_state_matrix(
        val_files,
        f'/scratch/almo2783/scratch/dim-less/8sensors/noise/state-matrix/state_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-snr-{snr_db}.npz',
        a, u_dc, mu, snr_db
    )

    build_state_matrix(
        test_files,
        f'/scratch/almo2783/scratch/dim-less/8sensors/noise/state-matrix/state_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-snr-{snr_db}.npz',
        a, u_dc, mu, snr_db
    )