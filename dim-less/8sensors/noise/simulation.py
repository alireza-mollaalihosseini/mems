...
def add_noise_with_snr(signal, snr_db):
    """
    Adds Gaussian noise to `signal` to achieve the specified SNR (in dB).
    """
    if snr_db is None:
        return signal  # no noise
...
def build_state_matrix(file_list_path, a, u_dc, mu, snr_db=None):
    ...
    for i, fname in enumerate(filenames):
        ...
        # --- Add noise here ---
        signal = add_noise_with_snr(signal, snr_db)
...
# Main
# -------------------------
def main(train_file_list_path, val_file_list_path, test_file_list_path, a, u_dc, mu, snr_db=None):

    lam = 1e4

    state_train = build_state_matrix(train_file_list_path, a, u_dc, mu, snr_db)
    state_val   = build_state_matrix(val_file_list_path, a, u_dc, mu, snr_db)
    state_test  = build_state_matrix(test_file_list_path, a, u_dc, mu, snr_db)
    ...
    # Save
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-snr-{snr_db}.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-snr-{snr_db}.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-snr-{snr_db}.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-snr-{snr_db}.txt",
               cm_val, fmt="%.5f")
