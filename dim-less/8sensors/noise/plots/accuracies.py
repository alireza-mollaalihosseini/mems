...
for snr_db in snr_db_values:
    ...
    for lam in lambda_values:
        # Load test accuracy
        result = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/lambda/results-a-{a}-u_dc-{u_dc:.2f}-lambda-{lam:.1e}-snr-{snr_db}.txt")
        test_accuracy = result[4]
        test_accuracies.append(test_accuracy * 100)

        # Load confusion matrix and calculate class accuracies
        conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/noise/results/lambda/conf_matrix-a-{a}-u_dc-{u_dc:.2f}-lambda-{lam:.1e}-snr-{snr_db}.txt")
    ...
    plt.savefig(f'lambda-optimization-without-legend-a-{a}-u_dc-{u_dc}-mu-{mu}-snr-{snr_db}.png', bbox_inches='tight', dpi=300)
    plt.close()
