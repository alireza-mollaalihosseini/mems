...
def process_lambda_value(X_train, X_val, labels_train, labels_val, lambda_value, a_value, u_dc_value, results_dir, snr_db):
    ...
    # Save results
    np.savetxt(f"{results_dir}/results-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}-snr-{snr_db}-200.txt",
               results.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}-snr-{snr_db}-200.txt",
               conf_matrix, fmt="%.5f")
