...
crossings = np.array([1.18510329, 0.59256307, 0.39510051, 0.29629138, 0.2370364 ,
                      0.19765198, 0.16938216, 0.1483405 , 0.13184918, 0.11852319])
# Preallocate
...
def process_u_dc(j, u_dc, a_values, const_tol, T, t_rec, u_max):
    results_up = np.load(
        f'/scratch/almo2783/scratch/dim-less/8sensors/490/grid-search/deflections-up/'
        f'deflections-RK4-up-t-sim-{int(T)}-t-rec-{int(t_rec)}-noise-2e-06-u_max-{u_max}-u_dc-{u_dc}.npy',
        mmap_mode="r"
    )
...
# save extrema_counts
np.save('extrema_count_490.npy', extrema_counts)
np.save('crossings_490.npy', crossings)
np.save('a_crits_490.npy', a_crits)
