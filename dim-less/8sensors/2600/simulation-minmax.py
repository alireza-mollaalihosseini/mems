...
def main(train_file_list_path, val_file_list_path, test_file_list_path, a, u_dc, mu):
    ...
    # # Get boolean mask
    # mask = state_train.std(axis=0) > 0.06
    # # Get indexes of columns that satisfy the condition
    # idx = np.where(mask)[0]
    std = state_train.std(axis=0)
    peaks, _ = find_peaks(std, height=0.006, distance=1)

    state_train = state_train[:, peaks]
    state_val   = state_val[:, peaks]
    state_test  = state_test[:, peaks]
    ...
    # Save
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-minmax.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-minmax.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-minmax.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-minmax.txt",
               cm_val, fmt="%.5f")
