...
for alpha in alpha_values:
    try:
        # Load test accuracy
        data = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/selected_features/results/results-a-{a_value}-alpha-{alpha}-lambda-{lambda_value}.txt")
        test_accuracy = data[4] * 100  # Convert to percentage
        test_accuracies.append(test_accuracy)

        # Load confusion matrix and extract per-class accuracies
        conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/selected_features/results/conf_matrix-a-{a_value}-alpha-{alpha}-lambda-{lambda_value}.txt")
