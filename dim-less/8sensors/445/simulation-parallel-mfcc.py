...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, omega_0, n_mfcc=16):
    ...
    # --- Replace FFT with MFCC ---
    # Librosa expects float32
    u_ac_buf = u_ac_buf.astype(np.float32)

    # Extract MFCCs
    mfccs = librosa.feature.mfcc(
        y=u_ac_buf, sr=int((1.0 * omega_0) / h), n_mfcc=n_mfcc, fmax=8000
    )

    # Option 1: Flatten (all coefficients × time frames)
    mfcc_features = mfccs.flatten()

    # Option 2: Take mean across time (feature vector of size n_mfcc)
...
def build_state_matrix(file_list_path, output_path, a, u_dc, mu):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(process_file)(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, omega_0, n_mfcc)
...
if __name__ == '__main__':
    ...
    build_state_matrix(
        train_files,
        f'/scratch/almo2783/scratch/dim-less/8sensors/445/state-matrix/state_matrix_train-a-{a}-u_dc-{u_dc}-mu-{mu}-mfcc.npz',
        a, u_dc, mu
    )

    build_state_matrix(
        val_files,
        f'/scratch/almo2783/scratch/dim-less/8sensors/445/state-matrix/state_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-mfcc.npz',
        a, u_dc, mu
    )

    build_state_matrix(
        test_files,
        f'/scratch/almo2783/scratch/dim-less/8sensors/445/state-matrix/state_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-mfcc.npz',
        a, u_dc, mu
    )