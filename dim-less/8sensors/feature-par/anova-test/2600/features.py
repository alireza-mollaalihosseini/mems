...
def top_features(X, y, top_k=10):
    y_labels = np.argmax(y, axis=1)

    # Apply ANOVA F-test
    selector = SelectKBest(score_func=f_classif, k=top_k)
    selector.fit(X, y_labels)
...
def process_top_k(top_k, X_train, X_val, labels_train, labels_val, lambda_value, a_value, u_dc_value, results_dir):

    extracted_features_idx = top_features(X_train, labels_train, top_k=top_k)
...
if __name__ == "__main__":
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/anova-test/2600/results"
    os.makedirs(results_dir, exist_ok=True)

    # Run in parallel
    Parallel(n_jobs=4)(  # adjust n_jobs depending on your cluster
