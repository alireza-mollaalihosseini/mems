...
for top_k in top_k_values:
    try:
        # Load test accuracy
        data = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/anova-test/2600/results/results-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-topk-{top_k}-lambda-{lambda_value}.txt")
        test_accuracy = data[4] * 100  # Convert to percentage
        test_accuracies.append(test_accuracy)

        # Load confusion matrix and extract per-class accuracies
        conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/anova-test/2600/results/conf_matrix-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-topk-{top_k}-lambda-{lambda_value}.txt")
