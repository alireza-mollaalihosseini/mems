...
types = np.array(['abs-weights', 'random-forrest'])
    ...
    for top_k in top_k_values:
        try:
            if typ=="abs-weights":
                ...
                data = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/{typ}/2600/results/results-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-topk-{top_k}-lambda-{lambda_value}-gini.txt")
...
# Save the figure
plt.savefig(f"accuracy_vs_nodes_lambda_1e4_weights-vs-gini.png", bbox_inches='tight')
plt.close()