...
def process_lambda_value(X_train, X_val, labels_train, labels_val, lambda_value, a_value, u_dc_value, results_dir):
    ...
    # Save results
    np.savetxt(f"{results_dir}/results-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}-minmax.txt",
               results.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}-minmax.txt",
               conf_matrix, fmt="%.5f")
...
if __name__ == "__main__":
    ...
    idx      = np.where(X_train.std(axis=0) > 1.0)[0]
