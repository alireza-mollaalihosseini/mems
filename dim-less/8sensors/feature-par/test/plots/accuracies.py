...
for lam in lambda_values:
    # Load test accuracy
    result = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/test/results/results-a-{a}-u_dc-{u_dc:.2f}-lambda-{lam:.1e}-corr-80-minmax.txt")
    test_accuracy = result[4]
    test_accuracies.append(test_accuracy * 100)

    # Load confusion matrix and calculate class accuracies
    conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/test/results/conf_matrix-a-{a}-u_dc-{u_dc:.2f}-lambda-{lam:.1e}-corr-80-minmax.txt")
...
plt.savefig(f'lambda-optimization-without-legend-a-{a}-u_dc-{u_dc}-mu-{mu}-corr-80-minmax.png', bbox_inches='tight', dpi=300)
plt.close()
