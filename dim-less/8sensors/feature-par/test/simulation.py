...
def main(a, u_dc, mu):
    ...
    state_train = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/state-matrix/state_matrix_train-a-0.44-u_dc-0.4-mu-1.0.npz")['arr_0']
    state_val   = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/state-matrix/state_matrix_val-a-0.44-u_dc-0.4-mu-1.0.npz")['arr_0']
    state_test  = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/state-matrix/state_matrix_test-a-0.44-u_dc-0.4-mu-1.0.npz")['arr_0']

    std      = state_train.std(axis=0)
    peaks, _ = find_peaks(std, height=0.006, distance=1)
    idx      = np.where(state_train.std(axis=0) > 1.0)[0]
    indices  = np.unique(np.concatenate([peaks, idx]))

    state_train = state_train[:, indices]
    state_val   = state_val[:, indices]
    state_test  = state_test[:, indices]
    ...
    # Save
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/test/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/test/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/test/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/test/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               cm_val, fmt="%.5f")
