...
def compute_feature_ranking_xgboost(X, y, n_estimators=200, learning_rate=0.1, max_depth=6, random_state=42, n_jobs=-1):
    """
    Compute feature ranking using XGBoost for multi-class.
    ...
    importances : np.ndarray
        Importance score for each feature (gain).
    """
    ...
    # Parameters (optimization: use 'hist' for faster training on high dims)
    params = {
    ...
    # Get importance (gain: contribution to loss reduction)
    importance_dict = model.get_score(importance_type='gain')
    importances = np.zeros(X.shape[1])
    for f, score in importance_dict.items():
        importances[int(f[1:])] = score  # f0 -> index 0
...
if __name__ == "__main__":
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/xgboost/2600/results"
    os.makedirs(results_dir, exist_ok=True)

    # Compute RF scores once
    ranked_idx, rf_scores = compute_feature_ranking_xgboost(X_train, labels_train)
