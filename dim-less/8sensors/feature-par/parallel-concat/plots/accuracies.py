...
for top_k in top_k_values:
    try:
        # Load test accuracy
        data = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/parallel-concat/results/results_test-topk-{top_k}-lambda-{lambda_value}.txt")
        test_accuracy = data[2] * 100  # Convert to percentage
        test_accuracies.append(test_accuracy)

        # Load confusion matrix and extract per-class accuracies
        conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/parallel-concat/results/conf_matrix_test-topk-{top_k}-lambda-{lambda_value}.txt")
...
# Save the figure
plt.savefig(f"accuracy_vs_nodes_lambda_1e4-teal-orange-without-legend_test.png", bbox_inches='tight')
# plt.show()
plt.close()