...
def process_length(length, lam, base_dir, results_dir):
    # Load feature indices
    extracted_features_idx = np.load(
        f'{base_dir}/overall_feature_idx_{int(length)}.npy'
    )

    # Slice features
    X_train1 = X_train1_full[:, extracted_features_idx]
    X_val1   = X_val1_full[:, extracted_features_idx]
    X_test1  = X_test1_full[:, extracted_features_idx]

    X_train7 = X_train7_full[:, extracted_features_idx]
    X_val7   = X_val7_full[:, extracted_features_idx]
    X_test7  = X_test7_full[:, extracted_features_idx]
    ...
    # Save results
    np.savetxt(f"{results_dir}/results_test-len-{length}-lam-{lam:.0e}-concat-2.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix_test-len-{length}-lam-{lam:.0e}-concat-2.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"{results_dir}/results_val-len-{length}-lam-{lam:.0e}-concat-2.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix_val-len-{length}-lam-{lam:.0e}-concat-2.txt",
               cm_val, fmt="%.5f")
...
if __name__ == "__main__":
    # Parameters
    lam = 1e4
    lengths = np.array([192, 200, 812, 835, 1057, 1923, 2823, 3579, 4205, 4696, 5097])
    results_dir = "/scratch/almo2783/scratch/dim-less/8sensors/feature-par/results"
    base_dir = "/scratch/almo2783/scratch/dim-less/8sensors/feature-par/feature_indices"
    ...
    # Parallel execution over lengths
    processed = Parallel(n_jobs=-1, verbose=10)(
        delayed(process_length)(
            length, lam, base_dir, results_dir
        )
