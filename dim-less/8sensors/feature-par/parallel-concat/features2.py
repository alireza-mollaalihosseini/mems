...
def process_top_k_and_sensors(top_k, lambda_value, results_dir, sensor_subset,
    ...
    # Ridge regression
    results_test, cm_test = ridge_regression_fast(X_train, labels_train, X_test, labels_test, lambda_value)
    results_val, cm_val   = ridge_regression_fast(X_train, labels_train, X_val, labels_val, lambda_value)

    # Name subset for saving
    subset_name = "-".join(map(str, sensor_subset))

    # Save results
    np.savetxt(f"{results_dir}/results_test-sensors-{subset_name}-topk-{top_k}-lambda-{lambda_value}.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix_test-sensors-{subset_name}-topk-{top_k}-lambda-{lambda_value}.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"{results_dir}/results_val-sensors-{subset_name}-topk-{top_k}-lambda-{lambda_value}.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix_val-sensors-{subset_name}-topk-{top_k}-lambda-{lambda_value}.txt",
               cm_val, fmt="%.5f")
...
if __name__ == "__main__":
    ...
    results_dir = f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/parallel-concat/results/combo"
    os.makedirs(results_dir, exist_ok=True)

    # # Explore all sensor subsets (size >=1)
    # all_subsets = []
    # sensors = list(sensor_data.keys())
    # for r in range(1, len(sensors)+1):
    #     all_subsets.extend(itertools.combinations(sensors, r))
    ...
    #         top_k, lambda_value, results_dir, subset, sensor_data, sensor_weights,
    #         labels_train, labels_test, labels_val
    #     )
