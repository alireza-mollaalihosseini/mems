...
for lam in lambda_values:
    # Load test accuracy
    result = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/filter-like-parallel-sensors/results/results_test-lam-{lam:.0e}.txt")
    test_accuracy = result[4]
    test_accuracies.append(test_accuracy * 100)

    # Load confusion matrix and calculate class accuracies
    conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/8sensors/feature-par/filter-like-parallel-sensors/results/conf_matrix_test-lam-{lam:.0e}.txt")
...
plt.savefig(f'lambda-optimization-without-legend-test.png', bbox_inches='tight', dpi=300)
plt.close()
