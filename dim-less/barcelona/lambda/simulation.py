...
def main(train_file_list_path, val_file_list_path, test_file_list_path, a, u_dc, mu):
    ...
    for lam in lambda_values:
        ...
        # Save
        np.savetxt(f"/scratch/almo2783/scratch/dim-less/barcelona/lambda/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
                results_test.reshape(1, -1), fmt="%.5f")
        np.savetxt(f"/scratch/almo2783/scratch/dim-less/barcelona/lambda/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
                cm_test, fmt="%.5f")

        np.savetxt(f"/scratch/almo2783/scratch/dim-less/barcelona/lambda/results/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
                results_val.reshape(1, -1), fmt="%.5f")
        np.savetxt(f"/scratch/almo2783/scratch/dim-less/barcelona/lambda/results/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
                cm_val, fmt="%.5f")
