...
for lam in lambda_values:
    # Load test accuracy
    result = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/barcelona/lambda/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt")
    test_accuracy = result[4]
    test_accuracies.append(test_accuracy * 100)

    # Load confusion matrix and calculate class accuracies
    conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/dim-less/barcelona/lambda/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt")
