...
def process_top_k_shap(top_k, ranked_idx, X_train, X_val,
...
if __name__ == "__main__":
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/test/results-shap"
    os.makedirs(results_dir, exist_ok=True)

    # load scores
    ranked_idx = np.load("/scratch/almo2783/scratch/test/shap_feature_ranking_idx.npy")
    rf_scores = np.load("/scratch/almo2783/scratch/test/shap_feature_importance.npy")

    # Run in parallel
    results = Parallel(n_jobs=64, backend="multiprocessing", verbose=1)(
        delayed(process_top_k_shap)(
            top_k, ranked_idx,
            X_train, X_val,
            labels_train, labels_val,
            lambda_values,
