...
if __name__ == '__main__':
    ...
    np.savez_compressed("state-matrix-64-sens.npz", state_matrix)
    ...
    # # split results and weights
    # outputs  = [r[0] for r in parallel_results]
    # weights  = [r[1] for r in parallel_results]
    ...
    # print(f"Train accuracy:{best_train}")
    # print(f"Validation accuracy:{best_test}")
    ...
    # W_no_bias = W_best[:-1, :]   # (n_features, n_classes)
    # bias      = W_best[-1, :]

    # X_background = X_train_std
    # X_mean = X_background.mean(axis=0)

    # # X_test_std: (n_samples, n_features)

    # shap_values = np.einsum(
    #     'ij,jk->ijk',
    #     X_test_std - X_mean,
    #     W_no_bias
    # )

    # y_pred = np.argmax(X_test_std @ W_no_bias + bias, axis=1)

    # shap_pred_class = np.array([
    #     shap_values[i, :, y_pred[i]]
    #     for i in range(len(y_pred))
    # ])

    # mean_abs_shap = np.mean(np.abs(shap_pred_class), axis=0)


    # feature_names = []
    # for f in f_values:
    #     for s in range(60):
    #         feature_names.append(f"f={int(f)}_ch{s}")
    ...
    #     values=shap_pred_class,          # (n_samples, n_features)
    #     base_values=np.zeros(len(X_test_std)),  # baseline already absorbed
    ...
    # shap.plots.bar(explainer_output, max_display=30)

    # shap.plots.beeswarm(explainer_output, max_display=30)

    # shap.plots.waterfall(explainer_output[0], max_display=20)
    # shap.plots.waterfall(explainer_output[1], max_display=20)

    # shap.plots.heatmap(explainer_output[:1000])

    # shap.initjs()
    # shap.force_plot(
    #     explainer_output.base_values[0],
    #     explainer_output.values[0],
    #     explainer_output.data[0],
    #     feature_names=feature_names
    # )
    ...
    # expl_class0 = shap.Explanation(
    #     values=shap_values[:, :, class_id],
    #     base_values=np.zeros(len(X_test_std)),
    #     data=X_test_df.values,
    #     feature_names=feature_names
    # )

    # shap.plots.bar(expl_class0)


    
    # top_k = 30
    # idx = np.argsort(mean_abs_shap)[-top_k:]
    ...
    # plt.title("Top SHAP Feature Importances (Ridge)")
    # plt.tight_layout()
    # plt.savefig("shap-top.png", dpi=300)
    # plt.close()


    # shap_freq = mean_abs_shap.reshape(len(f_values), 60).sum(axis=1)

    # plt.figure(figsize=(16, 8))
    # plt.plot(f_values, shap_freq)
    # plt.xlabel("Frequency (Hz)")
    # plt.ylabel("Total |SHAP|")
    # plt.title("Frequency Importance via SHAP")
    # plt.grid(True)
    # plt.savefig("feature-importance-shap.png", dpi=300)
    # plt.close()


    # plt.figure(figsize=(16, 8))
    # for c in range(W_no_bias.shape[1]):
    #     shap_c = np.mean(np.abs(shap_values[:, :, c]), axis=0)
    #     shap_freq_c = shap_c.reshape(len(f_values), 60).sum(axis=1)

    #     plt.plot(f_values, shap_freq_c, label=f"Class {c}")

    # plt.legend()
    # plt.xlabel("Frequency")
    # plt.ylabel("|SHAP|")
    # plt.title("Class-wise Frequency Importance")
    # plt.savefig("class-wise-features-shape.png", dpi=300)
    # plt.close()