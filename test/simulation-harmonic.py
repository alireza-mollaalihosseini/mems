...
#     y_final = simulate_transient_linear(1_500_000, h, c2)
...
#     np.save(f"best_lambda-harmonic-oscillators.npy", np.array(best_lambda))
#     np.save(f"training_acc-harmonic-oscillators.npy", np.array(best_train))
#     np.save(f"validation_acc-harmonic-oscillators.npy", np.array(best_val))
...
# =============================================================================
@njit(fastmath=True)
def rk4_step_inplace_linear(y, h, c1, c2, k1, k2, k3, k4, y_temp):
    exsi, etta = y
    k1[0] = etta
    k1[1] = -c2 * etta - c1 * exsi

    for i in range(2):
        y_temp[i] = y[i] + 0.5 * h * k1[i]
    exsi, etta = y_temp
    k2[0] = etta
    k2[1] = -c2 * etta -  c1 *exsi

    for i in range(2):
        y_temp[i] = y[i] + 0.5 * h * k2[i]
    exsi, etta = y_temp
    k3[0] = etta
    k3[1] = -c2 * etta -  c1 *exsi

    for i in range(2):
        y_temp[i] = y[i] + h * k3[i]
    exsi, etta = y_temp
    k4[0] = etta
    k4[1] = -c2 * etta -  c1 *exsi
...
@njit(fastmath=True)
def rk4_step_inplace_with_force_linear(y, h, c1, c2, c5, k1, k2, k3, k4, y_temp, f_x):
    exsi, etta = y
    k1[0] = etta
    k1[1] = -c2 * etta - c1 * exsi + c5 * f_x

    for i in range(2):
        y_temp[i] = y[i] + 0.5 * h * k1[i]
    exsi, etta = y_temp
    k2[0] = etta
    k2[1] = -c2 * etta - c1 * exsi + c5 * f_x

    for i in range(2):
        y_temp[i] = y[i] + 0.5 * h * k2[i]
    exsi, etta = y_temp
    k3[0] = etta
    k3[1] = -c2 * etta - c1 * exsi + c5 * f_x

    for i in range(2):
        y_temp[i] = y[i] + h * k3[i]
    exsi, etta = y_temp
    k4[0] = etta
    k4[1] = -c2 * etta - c1 * exsi + c5 * f_x
...
# =============================================================================
@njit(fastmath=True)
def simulate_transient_linear(N, h, c1, c2):
    y = np.zeros(2, dtype=np.float64)
    y[0] = 1e-9                     # tiny initial displacement
    k1 = np.zeros(2, dtype=np.float64)
    k2 = np.zeros(2, dtype=np.float64)
    ...
    for _ in range(N):
        rk4_step_inplace_linear(y, h, c1, c2, k1, k2, k3, k4, y_temp)
    return y


@njit(fastmath=True)
def simulate_with_force_linear(y, N, h, c1, c2, c5, f_ext):
    """y = transient result (2 states). Output = ξ (exsi)"""
    k1 = np.zeros(2, dtype=np.float64)
    k2 = np.zeros(2, dtype=np.float64)
    k3 = np.zeros(2, dtype=np.float64)
    ...
    for k in range(N):
        f_x = f_ext[k]
        rk4_step_inplace_with_force_linear(y, h, c1, c2, c5, k1, k2, k3, k4, y_temp, f_x)
        buf_u_ac[k] = y[0]          # ← pure exsi (no phi_ac)
    return buf_u_ac
...
def compute_transient(parameters, mu=1.0):
    ...
    y_final = simulate_transient_linear(30_000_000, h, c1, c2)
    return (y_final, h, c1, c2, c5)      # 4-tuple now
...
def process_one_file(fname, precomp_list):
    ...
    for i, precomp in enumerate(precomp_list):
        y_final, h, c1, c2, c5 = precomp          # 4 values
        u_ac_buf = simulate_with_force_linear(y_final, new_len, h, c1, c2, c5, signal)
        feats = extract_features(u_ac_buf)
        file_features[i*60:(i+1)*60] = feats
...
if __name__ == '__main__':
    ...
    u_dc = 1.0
    # f_values = np.linspace(1000, 50000, 100, dtype=int)
    f_values = np.linspace(10, 50000, 250, dtype=int)
