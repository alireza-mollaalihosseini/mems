...
# ===================================================================
# 1. Hermite cubic interpolation (precise, O(h³) local)
# ===================================================================
@njit(fastmath=True)
def hermite_interpolate(s, buf_phi, buf_dphi, max_index, h, initial_phi):
    if s <= 0.0:
        return initial_phi
    m = int(s / h)
    frac = (s - float(m) * h) / h
    if m >= max_index:
        return buf_phi[max_index]
    frac2 = frac * frac
    frac3 = frac * frac2
    h00 = 2.0 * frac3 - 3.0 * frac2 + 1.0
    h10 = frac3 - 2.0 * frac2 + frac
    h01 = -2.0 * frac3 + 3.0 * frac2
    h11 = frac3 - frac2
    p = (h00 * buf_phi[m] +
         h10 * h * buf_dphi[m] +
         h01 * buf_phi[m + 1] +
...
# ===================================================================
# 2. RK4 step WITHOUT delay (only for Phase 1)
# ===================================================================
@njit(fastmath=True)
...
# ===================================================================
# 3. RK4 step WITH delay in min-term + external force (used for Phase 2 & 3)
# ===================================================================
@njit(fastmath=True)
def rk4_step_inplace_with_delay_and_force(
    y, h, c1, c2, c3, c4, c5, phi_dc, a,
    k1, k2, k3, k4, y_temp,
    tau, buf_phi, buf_dphi, current_step, initial_phi, f_x
):
    t = current_step * h
    exsi, etta, psy, phi_ac = y
    ...
    # k1 at time t
    delayed_t = t - tau
    delayed_phi = hermite_interpolate(delayed_t, buf_phi, buf_dphi, current_step, h, initial_phi)
    temp = a * delayed_phi + phi_dc
    min_term = min(temp * temp, 1.0)
    ...
    # k2 & k3 at t + 0.5 h  (same delayed value)
    delayed_t = t + 0.5 * h - tau
    delayed_phi = hermite_interpolate(delayed_t, buf_phi, buf_dphi, current_step, h, initial_phi)
    temp = a * delayed_phi + phi_dc
    min_term = min(temp * temp, 1.0)
...
# ===================================================================
# 4. Phase 1 – Transient WITHOUT delay (builds clean initial history)
# ===================================================================
@njit(fastmath=True)
def simulate_transient_no_delay_with_history(N, h, c1, c2, c3, c4, phi_dc, a):
    y = np.zeros(4)
    y[0] = 1e-9
    k1 = np.zeros(4)
    k2 = np.zeros(4)
    ...
    buf_phi = np.zeros(N + 1)
    buf_dphi = np.zeros(N + 1)

    buf_phi[0] = y[3]
    buf_dphi[0] = -c3 * y[3] + c4 * y[1]

    for k in range(N):
        rk4_step_inplace(y, h, c1, c2, c3, c4, phi_dc, a, k1, k2, k3, k4, y_temp)
        buf_phi[k + 1] = y[3]
        buf_dphi[k + 1] = -c3 * y[3] + c4 * y[1]

    return y, buf_phi, buf_dphi


# ===================================================================
# 5. Phase 2 – Transient WITH delay, NO external force
# ===================================================================
@njit(fastmath=True)
def simulate_transient_with_delay_no_force(y_start, N, h, c1, c2, c3, c4, c5,
                                           phi_dc, a, tau,
                                           buf_phi_prev, buf_dphi_prev, N_prev):
    y = y_start.copy()
    k1 = np.zeros(4)
    k2 = np.zeros(4)
    k3 = np.zeros(4)
    ...
    buf_phi = np.zeros(N_prev + N + 1)
    buf_dphi = np.zeros(N_prev + N + 1)
    for i in range(N_prev + 1):
        buf_phi[i] = buf_phi_prev[i]
        buf_dphi[i] = buf_dphi_prev[i]

    initial_phi = 0.0

    for k in range(N):
        f_x = 0.0
        current_step = N_prev + k

        rk4_step_inplace_with_delay_and_force(
            y, h, c1, c2, c3, c4, c5, phi_dc, a,
            k1, k2, k3, k4, y_temp,
            tau, buf_phi, buf_dphi, current_step, initial_phi, f_x
        )

        buf_phi[N_prev + k + 1] = y[3]
        buf_dphi[N_prev + k + 1] = -c3 * y[3] + c4 * y[1]

    return y, buf_phi, buf_dphi


# ===================================================================
# 6. Phase 3 – Forced simulation WITH delay (your final output)
# ===================================================================
@njit(fastmath=True)
def simulate_with_force_and_delay(y_start, N, h, c1, c2, c3, c4, c5,
                                  phi_dc, a, f_ext, tau,
                                  buf_phi_prev, buf_dphi_prev, N_prev):
    y = y_start.copy()
    k1 = np.zeros(4)
...
def compute_transient(parameters, mu=1.0):
    ...
    # Phase 1: clean history without delay
    y1, buf_phi1, buf_dphi1 = simulate_transient_no_delay_with_history(
        N1, h, c1, c2, c3, c4, phi_dc, a
    )

    # Phase 2: turn delay ON, still no external force
    y2, buf_phi2, buf_dphi2 = simulate_transient_with_delay_no_force(
        y1, N2, h, c1, c2, c3, c4, c5, phi_dc, a, delay_tau,
        buf_phi1, buf_dphi1, N1
    )

    return (y2, buf_phi2, buf_dphi2, h, c1, c2, c3, c4, c5, phi_dc, a, N1, N2, delay_tau)
...
def process_one_file(fname, precomp_list):
    ...
    for i, (y2, buf_phi2, buf_dphi2, h, c1, c2, c3, c4, c5, phi_dc, a, N1, N2, delay_tau) in enumerate(precomp_list):
        u_ac_buf = simulate_with_force_and_delay(y2, N3, h, c1, c2, c3, c4, c5, phi_dc, a, signal, delay_tau, buf_phi2, buf_dphi2, N1 + N2)
...
if __name__ == '__main__':
    ...
    mu = 1.0
    delay_tau_ratios = np.array([2, 5, 10, 50, 100, 1000, 10000, 100000])
    a = 0.9
    u_dc = 1.0
    ...
    np.save(f"/scratch/almo2783/scratch/test/delay/lambdas/best_lambdas_u_dc-{u_dc:.1f}.npy", np.array(lambdas))
    np.save(f"/scratch/almo2783/scratch/test/delay/training/training_acc_u_dc-{u_dc:.1f}.npy", np.array(train_accs))
    np.save(f"/scratch/almo2783/scratch/test/delay/validation/validation_acc_u_dc-{u_dc:.1f}.npy", np.array(val_accs))