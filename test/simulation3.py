...
if __name__ == '__main__':
    ...
    precomp_list = np.load("precomputed_params.npy", allow_pickle=True)

    # Main parallel processing over files
    ...
    # # Precompute all frequency-dependent parameters (tiny memory)
    # precomp_list = []
    # N_trans = 50000000  # fixed
    # N_force = 1000000   # fixed

    # for f in f_values:
    #     omega_0 = f * 2 * np.pi
    #     h = 1e-6 * omega_0
    #     T_trans = 50.0 * omega_0

    #     N_trans_actual = int(T_trans / h)  # should be exactly 50M
    ...
    #     y_final = simulate_transient(N_trans_actual, h, c1, c2, c3, c4, phi_dc, a)
    #     precomp_list.append((y_final, h, c1, c2, c3, c4, c5, phi_dc))
    ...
    results = ridge_regression_fast(X_train_std, sub_labels_train, X_val_std, sub_labels_val, 1e2)
