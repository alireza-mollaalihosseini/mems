...
extrema_counts = {
    f: np.load(f"/scratch/almo2783/scratch/test/a-crit/extrems/extrema_counts_f_{f}.npy")
    for f in freqs
}
...
for i in range(a_crit.shape[1]):
  np.save(f"/scratch/almo2783/scratch/test/a-crit/a-crits/a-crit-u-dc-{u_dc_values[i]:.1f}.npy", a_crit[:, i])