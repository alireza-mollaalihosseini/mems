...
if __name__ == "__main__":
    ...
    np.save(f"/scratch/almo2783/scratch/test/coupled/grid-search/bif-maxima/maxima-counts-coupled-sensors-f1-{int(f1)}-f2-{int(f2)}-Q_0-{int(Q_0)}-(++-+).npy", result_grid)


    for i in range(len(a_vals)):
        for j in range(len(g_vals)):
            for k in range(len(u_dc_vals)):
    ...
    max_show = result_grid.max()

    pcm = ax.pcolormesh(a_vals, g_vals, result_grid[:,:,0].T, cmap="viridis", shading="nearest")
    cbar = fig.colorbar(pcm, ax=ax, fraction=0.046, pad=0.02, ticks=range(max_show + 1))
    cbar.ax.tick_params(labelsize=30)
    plt.xlabel("a values (feedback strength)", fontsize=30)
    plt.ylabel("g values (coupling strength)", fontsize=30)
    plt.xticks(fontsize=30)
    plt.yticks(fontsize=30)
    # plt.show()
    plt.tight_layout()
    plt.savefig(f"/scratch/almo2783/scratch/test/coupled/grid-search/bif-maxima/bifurcation-map-coupled-sensors-f1-{int(f1)}-f2-{int(f2)}-Q_0-{int(Q_0)}-(++-+).png", dpi=300)
    plt.close()