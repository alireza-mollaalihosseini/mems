...
for lam in lambda_values:
    ...
    result = np.loadtxt(f"/scratch/almo2783/scratch/audio-reg/results/results-test-lambda-{lam:.1e}.txt")
    test_accuracy = result[4]
    test_accuracies.append(test_accuracy * 100)

    # Load confusion matrix and calculate class accuracies
    # conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/audio-reg/results/conf_matrix-lambda-{lam:.1e}.txt")
    conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/audio-reg/results/conf_matrix-test-lambda-{lam:.1e}.txt")
...
plt.savefig(f'lambda-optimization-test.png', bbox_inches='tight', dpi=300)
plt.close()