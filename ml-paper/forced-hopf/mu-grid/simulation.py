...
def build_state_matrix(train_file_list_path, val_file_list_path, test_file_list_path, alpha, beta, lam, mu, fs=44100):
    ...
    val_state   = state_matrix[n_train:val_end]
    test_state  = state_matrix[val_end:]
...
if __name__ == '__main__':
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/forced-hopf/mu-grid/results"
    os.makedirs(results_dir, exist_ok=True)

    n_lams = len(lambda_values)

    train_state, val_state, test_state = build_state_matrix(train_files, val_files, test_files, alpha, beta, lam, mu)
    ...
    # Save raw metrics
    np.savetxt(
        f"{results_dir}/results-mu-{mu:.2e}-lam-{lam}-alpha-{alpha}-beta-{beta}.txt",
        outputs_arr,
        fmt="%.6f",
