...
def ridge_regression_fast(X_train, Y_train, X_eval, Y_eval, lam, alpha, beta):
    # Add bias term
    ...
    results = np.array([alpha, beta, lam, train_accuracy, accuracy, precision, recall, f1], dtype=np.float64)
    
    return results, conf_matrix


def process_lambda_value(X_train, X_val, labels_train, labels_val, lambda_value, alpha, beta, lam, results_dir):

    results, conf_matrix = ridge_regression_fast(
        X_train, labels_train, X_val, labels_val, lambda_value, alpha, beta
    )

    # Save results
    np.savetxt(f"{results_dir}/results-alpha-{alpha}-beta-{beta}-lam-{lam}-lambda-{lambda_value:.1e}.txt",
               results.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix-alpha-{alpha}-beta-{beta}-lam-{lam}-lambda-{lambda_value:.1e}.txt",
               conf_matrix, fmt="%.5f")
...
if __name__ == "__main__":
    ...
    X_train = np.load(f"/scratch/almo2783/scratch/ml-paper/forced-hopf/state-matrix/state_matrix_train-alpha--2.0-beta-1.2-lam-0.05-mu-0.8.npz")['arr_0']
    X_val   = np.load(f"/scratch/almo2783/scratch/ml-paper/forced-hopf/state-matrix/state_matrix_val-alpha--2.0-beta-1.2-lam-0.05-mu-0.8.npz")['arr_0']
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/forced-hopf/results"
    os.makedirs(results_dir, exist_ok=True)

    # Run in parallel
    Parallel(n_jobs=64, backend="threading", verbose=1)(  # adjust n_jobs depending on your cluster
        delayed(process_lambda_value)(
            X_train, X_val, labels_train, labels_val,
            lambda_value, alpha, beta, lam, results_dir
        )
