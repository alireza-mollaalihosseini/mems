...
if __name__ == '__main__':
    ...
    pca_components = [0.95, 0.99, 0.999, 500, 1000, 2000]
    ...
    for comp in pca_components:
        ...
        print(f"\n=== PCA with {comp} components (explained var: {pca.explained_variance_ratio_.sum():.4f}) ===")
        
        # Reuse your existing ridge_regression_fast (with or without soft)
        outputs = Parallel(n_jobs=64, verbose=1, backend="multiprocessing")(
            delayed(ridge_regression_fast)(
                X_train_pca, labels_train,
                X_val_pca, labels_val,
                lam
            ) for lam in lambda_values
        )

        outputs_arr = np.vstack(outputs)
        max_idx = np.argmax(outputs_arr[:, 2])
        max_val_acc = outputs_arr[max_idx, 2]
        print(f"Best val_acc for this PCA: {max_val_acc:.6f}")
        
        if max_val_acc > best_val_acc:
            best_val_acc = max_val_acc
            best_pca = comp
            best_outputs = outputs_arr

    print(f"\nOverall best val_acc: {best_val_acc:.6f} with PCA components = {best_pca}")
