...
def build_state_matrix(file_list_path, a, u_dc, mu):
    ...
    results = Parallel(n_jobs=64, verbose=10, backend='threading')(
...
# Main
# -------------------------
def main(train_file, val_file, test_file, label_train, label_val, label_test, a, u_dc, mu, typ):
    
    lam = 1e4
    
    state_train, state_val, state_test = (build_state_matrix(p, a, u_dc, mu) for p in [train_file, val_file, test_file])
    labels_train, labels_val, labels_test = (np.load(p) for p in [label_train, label_val, label_test])
    
    scaler = StandardScaler()
    state_train_std = scaler.fit_transform(state_train)
    state_val_std, state_test_std = scaler.transform(state_val), scaler.transform(state_test)
    ...
    # Save
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/increased-dataset/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-{typ}.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/increased-dataset/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-{typ}.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/increased-dataset/results/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-{typ}.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/increased-dataset/results/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}-{typ}.txt",
               cm_val, fmt="%.5f")


if __name__ == '__main__':
    a, u_dc, mu = (map(float, sys.argv[1:4]) if len(sys.argv) > 1 else (0.44, 0.4, 1.0))

    for typ in ["barcelona", "3cities", "5cities"]:
        main(
            f'/scratch/almo2783/scratch/rayson/design1/{typ}/train-filenames-{typ}-rayson.csv',
            f'/scratch/almo2783/scratch/rayson/design1/{typ}/val-filenames-{typ}-rayson.csv',
            f'/scratch/almo2783/scratch/rayson/design1/barcelona/test-filenames-barcelona-rayson.csv',
            f'/scratch/almo2783/scratch/ml-paper/increased-dataset/{typ}/labels_train.npy',
            f'/scratch/almo2783/scratch/ml-paper/increased-dataset/{typ}/labels_val.npy',
            f'/scratch/almo2783/scratch/ml-paper/increased-dataset/{typ}/labels_test.npy',
            a, u_dc, mu, typ
        )