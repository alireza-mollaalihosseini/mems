...
sensor_params = [
    ...
    (12.5, 1109.0, 6968.052505662161, 57.5, 0.0051, 196.07843137254903, 2669280.0, 12.92, 400980.0, 0.06, 1.0),
    (12.5, 1161.0, 7294.778141635499, 57.0, 0.0057, 175.43859649122805, 2391680.0, 13.23, 361630.0, 0.44, 0.1),
    (12.5, 2600.0, 16336.281798666923, 82.0, 0.0091, 109.89010989010988, 26827200.0, 50.24, 884700.0, 0.44, 0.4)
]
...
def main_all_sensors(train_file_list_path, val_file_list_path, test_file_list_path, mu):
    """
    Simulates 7 sensors, each with its own parameter set.
    The first 6 use fixed a, u_dc; the 7th uses a sweep over np.linspace(-2, 2, 101).
    """
    lam = 1e4
    # u_dc_fixed = 0.7
    # a_fixed = 0.5
    ...
    for idx, (alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa, a, u_dc) in enumerate(sensor_params):
        ...
        if idx < 6:
            ...
            X_train_all.append(freq_band_extract(state_train, freq_bands[idx]))
            X_test_all.append(freq_band_extract(state_test, freq_bands[idx]))
            X_val_all.append(freq_band_extract(state_val, freq_bands[idx]))

        # --- For 7th sensor (special case) ---
        else:
            u_dc_special = 0.4
            # a_values = np.linspace(-2, 2, 101)
            a_values = np.linspace(-2, 2, 1)

            for a in a_values:
                print(f"   -> Sensor 7 at a={a:.2f}")
                state_train = build_state_matrix(train_file_list_path, alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa, a, u_dc_special, mu)
                state_test  = build_state_matrix(test_file_list_path, alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa, a, u_dc_special, mu)
                state_val   = build_state_matrix(val_file_list_path, alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa, a, u_dc_special, mu)
                X_train_all.append(freq_band_extract(state_train, freq_bands[idx]))
                X_test_all.append(freq_band_extract(state_test, freq_bands[idx]))
                X_val_all.append(freq_band_extract(state_val, freq_bands[idx]))

                # combine the state matrices
                ...
                # Save
                np.savetxt(f"/scratch/almo2783/scratch/ml-paper/filter-like-7/results/results_test-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
                        results_test.reshape(1, -1), fmt="%.5f")
                np.savetxt(f"/scratch/almo2783/scratch/ml-paper/filter-like-7/results/conf_matrix_test-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
                        cm_test, fmt="%.5f")

                np.savetxt(f"/scratch/almo2783/scratch/ml-paper/filter-like-7/results/results_val-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
                        results_val.reshape(1, -1), fmt="%.5f")
                np.savetxt(f"/scratch/almo2783/scratch/ml-paper/filter-like-7/results/conf_matrix_val-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
                        cm_val, fmt="%.5f")

    # # Combine all frequency segments
    # X_train_combined = np.concatenate(X_train_all, axis=1)
    # np.save("X_train_combined.npy", X_train_combined)
    # print("✅ Combined training matrix saved: X_train_combined.npy")
...
#     # Save
#     np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
#                results_test.reshape(1, -1), fmt="%.5f")
#     np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
#                cm_test, fmt="%.5f")

#     np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
#                results_val.reshape(1, -1), fmt="%.5f")
#     np.savetxt(f"/scratch/almo2783/scratch/dim-less/8sensors/2600/results/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
#                cm_val, fmt="%.5f")
