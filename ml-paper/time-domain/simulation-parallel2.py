...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a):
    ...
    downsampled = decimate(u_ac_buf, downsample_factor, ftype='fir')  # 'fir' for linear phase (recommended for signals)
...
# Main
# -------------------------
def main(train_file, val_file, test_file, label_train, label_val, label_test, a, u_dc, mu):
    ...
    # Save
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/time-domain/results/results_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/time-domain/results/conf_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               cm_test, fmt="%.5f")

    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/time-domain/results/results_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/time-domain/results/conf_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}-lam-{lam:.0e}.txt",
               cm_val, fmt="%.5f")
...
if __name__ == '__main__':
    ...
    main(
        ...
        '/scratch/almo2783/scratch/dim-less/barcelona/label_matrix_train.npy',
        '/scratch/almo2783/scratch/dim-less/barcelona/label_matrix_test.npy',
        '/scratch/almo2783/scratch/dim-less/barcelona/label_matrix_val.npy',
        a, u_dc, mu
    )