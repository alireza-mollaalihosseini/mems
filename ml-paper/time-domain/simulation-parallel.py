...
# fs = np.linspace(0, 16000, 10, dtype=int)

# Define the parameters for 8 sensors
sensor_params = [
    # alpha, f, omega_0, Q_0, tau, beta, gamma, R, kappa, a, u_dc
    (12.5, 2356.937294669354, 2 * np.pi * 1777, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 3737.2602676211245, 2 * np.pi * 3555, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 5950.388703003204, 2 * np.pi * 5333, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 7666.191077157651, 2 * np.pi * 7111, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 8769.686556611563, 2 * np.pi * 8888, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 10465.007213569997, 2 * np.pi * 10666, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 12079.336976921315, 2 * np.pi * 12444, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4),
    (12.5, 15697.296843819993, 2 * np.pi * 14222, 50, 0.001, 1066, 1.62e7, 12.5, 1e6, 0.5, 0.4)
]
...
def main_all_sensors(train_file_list_path, val_file_list_path, test_file_list_path, a, mu):
    ...
    # Save
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/no-equidist/8sens/results/results_test-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
            results_test.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/no-equidist/8sens/results/conf_matrix_test-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
            cm_test, fmt="%.5f")

    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/no-equidist/8sens/results/results_val-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
            results_val.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"/scratch/almo2783/scratch/ml-paper/no-equidist/8sens/results/conf_matrix_val-a-{a}-u_dc-{u_dc_special}-mu-{mu}-lam-{lam:.0e}.txt",
            cm_val, fmt="%.5f")
