...
#     np.savetxt(f"{results_dir}/results-a-{a_value:.2f}-lambda-{lambda_value}.txt", 
#                 results.reshape(1, -1), fmt="%.5f")
    
#     np.savetxt(f"{results_dir}/conf_matrix-a-{a_value:.2f}-lambda-{lambda_value}.txt", 
#                 conf_matrix, fmt="%.5f")
...
def objective(trial, X_train, y_train_true, X_val, labels_train, labels_val, a_value, u_dc_value):
    ...
    X_train_sel = selector.fit_transform(X_train, y_train_true)
...
if __name__ == "__main__":
    ...
    # Prepare true labels for feature selection (1D, since one-hot encoded)
    y_train_true = np.argmax(labels_train, axis=1)
    y_val_true = np.argmax(labels_val, axis=1)
    ...
    study = optuna.create_study(
        study_name="ridge_feature_selection",
        direction="maximize",
    ...
    study.optimize(
        lambda trial: objective(trial, X_train, y_train_true, X_val, labels_train, labels_val, a_value, u_dc_value),
        n_trials=1000
    )
    ...
    # To save selected features, fit selector with best n_features
    selector = SelectKBest(score_func=f_classif, k=min(best_params['n_features'], X_train.shape[1]))
    selector.fit(X_train, y_train_true)
    ...
    np.savetxt(f"{results_dir}/best_results.txt", 
               best_results.reshape(1, -1), fmt="%.5f")
    
    np.savetxt(f"{results_dir}/best_conf_matrix.txt", 
