...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="ridge_feature_selection_weights_445",
        direction="maximize",
        storage=f"sqlite:///{results_dir}/optuna-weights-445.db",
        load_if_exists=True
    )
    ...
    np.savetxt(f"{results_dir}/best_results_weights_445.txt", 
               best_results.reshape(1, -1), fmt="%.5f")
    
    np.savetxt(f"{results_dir}/best_conf_matrix_weights_445.txt", 
