...
if __name__ == "__main__":
    ...
    X_train = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/582/state-matrix/state_matrix_train-a-0.02-u_dc-0.9-mu-1.0.npz")['arr_0']
    X_val   = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/582/state-matrix/state_matrix_val-a-0.02-u_dc-0.9-mu-1.0.npz")['arr_0']
    ...
    # Load weights
    weights = np.load(f"/scratch/almo2783/scratch/dim-less/8sensors/582/weights/weights-a-0.02-lambda-10000.0.npz")['arr_0']
    ...
    study = optuna.create_study(
        study_name="ridge_feature_selection_weights_582",
        direction="maximize",
        storage=f"sqlite:///{results_dir}/optuna-weights-582.db",
        load_if_exists=True
    )
    ...
    # df.to_csv(f"{results_dir}/optuna_results_weights_582.csv", index=False)
    ...
    np.savetxt(f"{results_dir}/best_results_weights_582.txt", 
               best_results.reshape(1, -1), fmt="%.5f")
    
    np.savetxt(f"{results_dir}/best_conf_matrix_weights_582.txt", 
