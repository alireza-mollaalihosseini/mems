...
for lam in lambda_values:
    # Load test accuracy
    result = np.loadtxt(f"/scratch/almo2783/scratch/ml-paper/short-time/results/results-a-{a}-u_dc-{u_dc:.2f}-lambda-{lam:.1e}.txt")
    test_accuracy = result[4]
    test_accuracies.append(test_accuracy * 100)

    # Load confusion matrix and calculate class accuracies
    conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/ml-paper/short-time/results/conf_matrix-a-{a}-u_dc-{u_dc:.2f}-lambda-{lam:.1e}.txt")
