...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len):
    ...
    # u_ac_buf = u_ac_buf[::500]
    
    diff = np.diff(u_ac_buf)[::500]
    diff2 = np.diff(u_ac_buf, n=2)[::500]

    # ---- Time-domain statistics ----
    mean_u = np.mean(u_ac_buf)
    std_u = np.std(u_ac_buf)
    ...
    zcr_diff = np.mean(np.diff(np.sign(diff)) != 0)

    # ---- Energy features ----
    energy_u = np.sum(u_ac_buf**2)
    energy_ratio = np.sum(diff**2) / (energy_u + 1e-12)

    return np.concatenate([u_ac_buf[::500], diff, diff2, np.array([mean_u, std_u, rms_u, ptp_u, skew_u, kurt_u, std_diff, std_diff2, zcr_diff, energy_u, energy_ratio])])
...
if __name__ == '__main__':
    ...
    build_state_matrix(
        train_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_train-time-with-diff.npz',
        a, u_dc, mu
    )

    build_state_matrix(
        val_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_val-time-with-diff.npz',
        a, u_dc, mu
    )

    build_state_matrix(
        test_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_test-time-with-diff.npz',
        a, u_dc, mu
    )