...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len, sr, feature_type="logmel"):
    ...
    # DC removal
    u_ac_buf -= np.mean(u_ac_buf)

    if feature_type == "logmel":
        return extract_logmel(u_ac_buf, sr).astype(np.float32)

    elif feature_type == "mfcc":
        return extract_mfcc_deltas(u_ac_buf, sr).astype(np.float32)

    else:
        raise ValueError(f"Unknown feature_type: {feature_type}")


def build_state_matrix(file_list_path, output_path, a, u_dc, mu, feature_type):
    ...
    results = Parallel(n_jobs=64, backend="multiprocessing", verbose=1)(
        delayed(process_file)(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len, new_sr, feature_type)
...
if __name__ == '__main__':
    ...
    build_state_matrix(
        train_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_train-logmel.npz',
        a, u_dc, mu,
        feature_type = "logmel"
    )
    ...
    build_state_matrix(
        val_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_val-logmel.npz',
        a, u_dc, mu,
        feature_type = "logmel"
    )
    ...
    build_state_matrix(
        test_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_test-logmel.npz',
        a, u_dc, mu,
        feature_type = "logmel"
    )
    ...
    build_state_matrix(
        train_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_train-mfcc.npz',
        a, u_dc, mu,
        feature_type = "mfcc"
    )
    ...
    build_state_matrix(
        val_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_val-mfcc.npz',
        a, u_dc, mu,
        feature_type = "mfcc"
    )
    ...
    build_state_matrix(
        test_files,
        f'/scratch/almo2783/scratch/ml-paper/states/state-matrix/state_matrix_test-mfcc.npz',
        a, u_dc, mu,
        feature_type = "mfcc"
    )