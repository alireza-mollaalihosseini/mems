...
def compute_full_nonlinearity(sim_dir, save_dir, a_values, fs=1e6, n_jobs=64):
    ...
    original_data = np.load(os.path.join(sim_dir, "avg_original_signal.npz"))
    x = original_data["signal"]

    def process_single_a(a):
        sim_path = os.path.join(sim_dir, f"avg_u_ac_a_{a:.2f}.npz")
...
save_dir = "/scratch/almo2783/scratch/ml-paper/nonlinearity/test"
a_values = np.linspace(-1, 1, 101)

rho, mi, nlr = compute_full_nonlinearity(sim_dir, save_dir, a_values, fs=1e6, n_jobs=64)
