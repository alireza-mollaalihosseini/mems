...
echo "Starting balanced job submission for ${#a_values[@]} a_values (u_dc=${u_dc}, mu=${mu}). Total: $max_total_jobs jobs."
echo "Target concurrent jobs: <= $max_concurrent_jobs in 'BatchXL' queue."
echo "Per-job resources: 64 cores, 300GB mem, 1hr walltime."
...
for a in "${a_values[@]}"; do
    ...
    bsub -q BatchXL \
        -n 64 \
        -o "${results_dir}/err-out/output_a_${a}.txt" \
        -e "${results_dir}/err-out/error_a_${a}.txt" \
        -J "ridge_a_${a}" \
        python3 simulation-10fold.py --a "${a}" \
        && echo "Submitted job for a=${a} (total submitted: $((++submitted_today)))" \
        || echo "Failed to submit job for a=${a}" >> "$failed_log"
