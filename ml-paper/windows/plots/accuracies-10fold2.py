...
# n_folds       = 10
# n_windows     = 3 # 10
# results_dir = "/scratch/almo2783/scratch/ml-paper/windows/results"
...
#             fpath = (
#                 f"{results_dir}/"
#                 f"fold_results-a-{a:.2f}-lambda-{lam:.1e}-fold-{fold}-win-{int(n_windows)}.txt"
#             )
...
# # save the values
# np.save(f"best_train_over_a-win-{int(n_windows)}.npy",best_train_a)
# np.save(f"best_train_std-win-{int(n_windows)}.npy",best_train_std)
# np.save(f"best_test_over_a-win-{int(n_windows)}.npy",best_test_a)
# np.save(f"best_test_std-win-{int(n_windows)}.npy",best_test_std)
# np.save(f"best_lambda_over_a-win-{int(n_windows)}.npy",best_lambda_a)
...
# plt.savefig(f"feedback-optimization-10folds-errorbars-win-{int(n_windows)}.png", dpi=300)
# plt.close()
