...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len_full, n_windows=10):
    ...
    for w in range(n_windows):
        ...
        log_megs = np.log10(np.abs(fft_vals[:actual_fft_len]))

        # coeffs = dct(mags, type=2, norm='ortho')
        # results.append(coeffs[:300])


        # results.append(mags[:600])
        results.append(log_megs)
...
if __name__ == '__main__':
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/windows/dct/test"
