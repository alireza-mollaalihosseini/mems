...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len_full, n_windows=10):
    ...
    # Store window spectra (n_windows, fft_len_win)
    mags_all = np.full((n_windows, fft_len_win), np.nan, dtype=np.float64)
    ...
    for w in range(n_windows):
        ...
        mags = np.log10(np.abs(fft_vals))
        
        n_use = min(len(mags), fft_len_win)
        mags_all[w, :n_use] = mags[:n_use]

    # -------------------------
    # Statistics across windows
    # -------------------------
    mags_mean = np.nanmean(mags_all, axis=0)
    mags_std  = np.nanstd(mags_all, axis=0)
    mags_max  = np.nanmax(mags_all, axis=0)

    # Final feature vector
    features = np.hstack([mags_mean, mags_std, mags_max])
    
    return features.astype(np.float32)


def build_state_matrix(train_file_list_path, val_file_list_path, test_file_list_path, a, u_dc, mu, n_windows=10):
    # Load filenames
...
if __name__ == '__main__':
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/windows/test"
    os.makedirs(results_dir, exist_ok=True)

    n_lams = len(lambda_values)

    n_windows = 3 # 10
    state_matrix = build_state_matrix(train_files, val_files, test_files, a, u_dc, mu, n_windows)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(state_matrix), start=1):
        ...
        for i, (metrics) in enumerate(fold_outputs):
            ...
            # Save raw metrics
            np.savetxt(
                f"{results_dir}/fold_results-a-{a:.2f}-lambda-{lam:.1e}-fold-{fold}-win-{int(n_windows)}.txt",
                metrics.reshape(1,-1),
                fmt="%.6f"
            )