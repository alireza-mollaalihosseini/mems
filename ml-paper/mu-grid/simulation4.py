...
def process_file(fname, idxs, b_filt, a_filt, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len):
    ...
    fft_mag = np.log10(np.abs(fft_vals[:fft_len]))
    envelope_smooth = savgol_filter(fft_mag, window_length = 51, polyorder=3)
    # log_megs = np.log10(np.abs(fft_vals))

    # mfccs = librosa.feature.mfcc(y=u_ac_buf, n_mfcc=13, sr=1000000)
    # delta_mfccs = librosa.feature.delta(mfccs)
    # delta2_mfccs = librosa.feature.delta(mfccs, order=2)
    # all_mfccs = np.concatenate((mfccs, delta_mfccs, delta2_mfccs))

    # return np.log10(np.abs(fft_vals[:fft_len])).astype(np.float32)
    # return mfccs.flatten().astype(np.float32)
    return envelope_smooth.astype(np.float32)


def build_state_matrix(train_file_list_path, val_file_list_path, a, u_dc, mu):
    # Load filenames
...
parser.add_argument('--mu', type=float, required=True, help='Value of mu to process')
...
if __name__ == '__main__':
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/mu-grid"
    os.makedirs(results_dir, exist_ok=True)

    train_state, val_state = build_state_matrix(train_files, val_files, a, u_dc, mu)
    ...
    # Save raw metrics
    np.savetxt(
        f"{results_dir}/results-a-{a:.2f}-mu-{mu:.2e}.txt",
        outputs_arr,
        fmt="%.6f",
