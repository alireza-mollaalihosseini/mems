...
#     np.save(f"/scratch/almo2783/scratch/test/a-crit/extrems/extrema_counts_f_{int(f_val)}-more2.npy", extrema_counts)

# extrema_counts = Parallel(n_jobs=-1, backend="loky", verbose=1)(
#     delayed(simulate_for_u_dc)(u_dc, a_values, f=f_val)
#     for f_val in f_values
# )

# extrema_counts = np.array(extrema_counts)
# np.save("/scratch/almo2783/scratch/test/a-crit/extrems/extrema_counts_u-dc-1.0.npy", extrema_counts)

for f_val in f_values:

    print(f"Simulating for f={int(f_val)} Hz...")
    extrema_counts = simulate_for_u_dc(u_dc, a_values, f=f_val)
    np.save(f"/scratch/almo2783/scratch/test/a-crit/extrems/extrema_counts_f_{int(f_val)}-u-dc-1.0.npy", extrema_counts)