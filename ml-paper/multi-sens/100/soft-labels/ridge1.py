...
if __name__ == '__main__':
    ...
    for k in range(num_tenths):
        ...
        outputs_k = Parallel(n_jobs=64, verbose=1, backend="multiprocessing")(
            delayed(ridge_regression_fast)(X_train_k, labels_train, X_val_k, labels_val, lam)
            for lam in lambda_values
        )
        outputs_arr_k = np.vstack(outputs_k)
    ...
    print(f"Best lambda for distilled model: {best_lam}")
