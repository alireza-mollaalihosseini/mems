...
if __name__ == '__main__':
    ...
    print("\n=== MLP Neural Net on hard labels ===")
    mlp_hard = MLPClassifier(
        hidden_layer_sizes=(512, 256),   # decent capacity for 6060 dims
        activation='relu',
        solver='adam',
    ...
    mlp_hard.fit(X_train_full_std, hard_y_train)

    test_hats_mlp_hard = mlp_hard.predict(X_test_full_std)
    test_acc_mlp_hard = accuracy_score(hard_y_test, test_hats_mlp_hard)
    print(f"MLP-hard test accuracy: {test_acc_mlp_hard:.4f}")

    print("\n=== MLP Neural Net with soft probability distillation (T=1.0) ===")
    # For soft labels, use MLPRegressor + multi-output (similar to XGBoost)
    ...
    mlp_soft = MultiOutputRegressor(
        MLPRegressor(
            hidden_layer_sizes=(512, 256),
            activation='relu',
            solver='adam',
            alpha=1e-3,                  # stronger L2 for soft targets
            batch_size=256,
            learning_rate='adaptive',
            max_iter=500,
            early_stopping=True,
    ...
    mlp_soft.fit(X_train_full_std, soft_probs_train)

    test_probs_mlp = np.clip(mlp_soft.predict(X_test_full_std), 0, 1)
    test_hats_mlp = np.argmax(test_probs_mlp, axis=1)

    test_acc_mlp = accuracy_score(hard_y_test, test_hats_mlp)
    print(f"MLP-soft test accuracy: {test_acc_mlp:.4f}")