...
def build_state_matrix(train_file_list_path, val_file_list_path, test_file_list_path, results_dir, a, u_dc, mu, f):
    ...
    # Transpose: list of tuples -> per-segment lists
    results_per_segment = list(zip(*results))

    # Stack into state matrices
    state_matrices = [
        np.vstack(segment_features)
        for segment_features in results_per_segment
    ]

    # Save
    os.makedirs(results_dir, exist_ok=True)
    for i, state_matrix in enumerate(state_matrices, start=1):
        output_path = f"{results_dir}/f-{int(f)}-{i}.npz"
...
if __name__ == '__main__':
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/multi-sens/100/results/one-tenth"
    os.makedirs(results_dir, exist_ok=True)
