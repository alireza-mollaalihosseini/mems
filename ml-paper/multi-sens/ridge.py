...
if __name__ == '__main__':
    ...
    # f_values = np.array([2000, 5000])
    # f_values = np.linspace(8000, 30000, 201)
    # f_values = np.arange(1000, 50000, 50)
    # f_values = np.linspace(1000, 50000, 201)
    f_values = np.linspace(1000, 50000, 101)

    state_matrix = np.zeros((10510, len(f_values) * 16))
    
    for i, f in enumerate(f_values):
        cols = np.load(f"/scratch/almo2783/scratch/ml-paper/multi-sens/results/a-{a:.2f}-u_dc-{u_dc:.2f}/f-{int(f)}.npz")["arr_0"]
    ...
    plt.savefig(f"/scratch/almo2783/scratch/ml-paper/multi-sens/plots/grid-lam-opt/lambda-optimization-a-{a:.2f}-u_dc-{u_dc:.2f}.png", dpi=300)
    plt.close()

    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/multi-sens/accuracy"
    ...
    # delete cols
    for i, f in enumerate(f_values):
        os.remove(f"/scratch/almo2783/scratch/ml-paper/multi-sens/results/a-{a:.2f}-u_dc-{u_dc:.2f}/f-{int(f)}.npz")

    err_out_dir = Path(
        f"/scratch/almo2783/scratch/ml-paper/multi-sens/results/a-{a:.2f}-u_dc-{u_dc:.2f}/err-out"
    )
