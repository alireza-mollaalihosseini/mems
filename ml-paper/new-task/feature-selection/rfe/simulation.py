...
def compute_feature_ranking_rfe(X, y):
    ...
    rfe = RFE(
        estimator=estimator,
        n_features_to_select=1,   # full ranking
        step=1                    # one-by-one → maximum precision
    )
    rfe.fit(X, y_labels)
    return np.argsort(rfe.ranking_), rfe.ranking_
...
if __name__ == "__main__":
    ...
    ranked_idx, importance = compute_feature_ranking_rfe(X_train, labels_train)

    # save
    np.save("idxes.npy", ranked_idx)
    np.save('importance.npy', importance)

    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/new-task/feature-selection/rfe/results"
