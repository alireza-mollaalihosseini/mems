...
def process_top_k(top_k, X_train, X_val, labels_train, labels_val, lambda_value, a_value, u_dc_value, results_dir, importance, bands):
    ...
    permutation_bands = bands[extracted_features_idx]  # shape (top_k, 2)

    # --- SORT BY START INDEX (ascending) ---
    # ensure numpy array of shape (k,2)
    permutation_bands = np.array(permutation_bands)
    order = np.argsort(permutation_bands[:, 0])
    permutation_bands = permutation_bands[order]

    # print(f'top_k={top_k}, Length of unique indices = {len(extracted_features_idx)}')
    X_train_selected = np.concatenate([X_train[:, start:end] for start, end in permutation_bands], axis=1)
    X_val_selected   = np.concatenate([X_val[:, start:end] for start, end in permutation_bands], axis=1)
...
if __name__ == "__main__":
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/new-task/feature-selection/permutation/results"
    os.makedirs(results_dir, exist_ok=True)

    importance  = np.load('/scratch/almo2783/scratch/ml-paper/new-task/permutation/bandwise_importance.npy')
    bands = np.load('/scratch/almo2783/scratch/ml-paper/new-task/permutation/bands.npy')
