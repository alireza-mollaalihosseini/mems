...
def process_lambda_value(X_train, X_val, labels_train, labels_val, lambda_value, a_value, u_dc_value, results_dir):
    ...
    # Save results
    np.savetxt(f"{results_dir}/results-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}.txt",
               results.reshape(1, -1), fmt="%.5f")
    np.savetxt(f"{results_dir}/conf_matrix-a-{a_value:.2f}-u_dc-{u_dc_value:.2f}-lambda-{lambda_value:.1e}.txt",
               conf_matrix, fmt="%.5f")
...
if __name__ == "__main__":
    ...
    X_train = np.load(f"/scratch/almo2783/scratch/ml-paper/new-task/state-matrix/state_matrix_train-a-0.44-u_dc-0.4-mu-1.0.npz")['arr_0']
    X_val   = np.load(f"/scratch/almo2783/scratch/ml-paper/new-task/state-matrix/state_matrix_val-a-0.44-u_dc-0.4-mu-1.0.npz")['arr_0']
    labels_train = np.load(f"/scratch/almo2783/scratch/ml-paper/new-task/filepaths/label_matrix_train.npy")
    labels_val   = np.load(f"/scratch/almo2783/scratch/ml-paper/new-task/filepaths/label_matrix_val.npy")
    ...
    # Results dir
    results_dir = f"/scratch/almo2783/scratch/ml-paper/new-task/results/lambda"
    os.makedirs(results_dir, exist_ok=True)

    # Run in parallel
    Parallel(n_jobs=64, backend="threading", verbose=10)(  # adjust n_jobs depending on your cluster
