...
def process_file(fname, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len):
...
def build_state_matrix(file_list_path, output_path, a, u_dc, mu):
    ...
    results = Parallel(n_jobs=64, backend='threading')(
        delayed(process_file)(fname, y_final, N_force, h, c1, c2, c3, c4, c5, phi_dc, a, fft_len)
...
if __name__ == '__main__':
    ...
    # Paths
    train_files = '/scratch/almo2783/scratch/ml-paper/new-task/filepaths/train-filenames.csv'
    val_files = '/scratch/almo2783/scratch/ml-paper/new-task/filepaths/val-filenames.csv'
    test_files = '/scratch/almo2783/scratch/ml-paper/new-task/filepaths/test-filenames.csv'

    build_state_matrix(
        train_files,
        f'/scratch/almo2783/scratch/ml-paper/new-task/state-matrix/state_matrix_train-a-{a}-u_dc-{u_dc}-mu-{mu}.npz',
        a, u_dc, mu
    )

    build_state_matrix(
        val_files,
        f'/scratch/almo2783/scratch/ml-paper/new-task/state-matrix/state_matrix_val-a-{a}-u_dc-{u_dc}-mu-{mu}.npz',
        a, u_dc, mu
    )

    build_state_matrix(
        test_files,
        f'/scratch/almo2783/scratch/ml-paper/new-task/state-matrix/state_matrix_test-a-{a}-u_dc-{u_dc}-mu-{mu}.npz',
        a, u_dc, mu
    )