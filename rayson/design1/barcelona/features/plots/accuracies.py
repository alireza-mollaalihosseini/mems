...
#         try:
#             # Load the data
#             data = np.loadtxt(f"/scratch/almo2783/scratch/rayson/design1/barcelona/features/results/results-a-{a_value:.2f}-alpha-{alpha}-lambda-{lambda_value}.txt")
#             test_accuracy = data[4] * 100
#             accuracies.append(test_accuracy)
#         except Exception as e:
#             accuracies.append(np.nan)
...
for alpha in alpha_values:
    try:
        ...
        conf_matrix = np.loadtxt(f"/scratch/almo2783/scratch/rayson/design1/barcelona/features/results/conf_matrix-a-{a_value:.2f}-alpha-{alpha}-lambda-{lambda_value}.txt")
...
plt.plot(nodes, test_accuracies, marker='o', linewidth=2, color='orangered', label="Test Accuracy")

# Define colormap and assign unique colors to each class
cmap_custom = LinearSegmentedColormap.from_list("custom_cmap", ["teal", "white", "darkorange"])
...
# plt.legend(title='Classes', fontsize=30, bbox_to_anchor=(1.0, 1.0), loc='upper left')
...
# Save the figure
plt.savefig(f"accuracy_vs_nodes_lambda_1e4-without-legend.png", dpi=900, bbox_inches='tight')
# plt.show()
plt.close()
...
# # Plot test accuracy in **black**
# plt.plot(nodes, test_accuracies, marker='o', linewidth=2, color='black', label="Test Accuracy")
...
# plt.xscale('log')  # Log scale for nodes
# plt.grid(True, linestyle='--', alpha=0.5)
# plt.xticks(nodes, labels=[f'{n}' for n in nodes], rotation=45, fontsize=20)
...
# # Save the figure
# plt.savefig("accuracy_vs_nodes_lambda_1e3-1-without-legend.png", dpi=900, bbox_inches='tight')
# plt.close()